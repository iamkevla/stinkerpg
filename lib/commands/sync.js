const debug = require('debug')('lib/commands/sync');
...
module.exports = function* (argv) {
  ...
  for (let table of tablesToSync) {
    ...
    let hasReportDateColumn = yield selectSource.query(`
    ...
    let { rows } = yield selectSource.query(`
    ...
    function wrapStrings(value) {
      if (typeof value === 'string') {
        ...
        return moment(value).tz("Australia/Sydney").format('LLLL');
      } else {
        return value;
      }
    }
      ...
      if (si.id === ti.id) {
        ...
        if (si.hash !== ti.hash) {
          ...
          try {
            ...
            let records = yield selectSource.query(`
              SELECT * FROM public."${table}" WHERE "id" = '${si.id}';
            `);
